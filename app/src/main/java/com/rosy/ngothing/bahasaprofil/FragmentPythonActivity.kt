...
class FragmentPythonActivity : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.activity_fragment_python, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Memuat kuis dengan kategori 'Python' dan status lulus
        loadPassedQuizzes("Python")
    }
    ...
    private suspend fun getQuizHistoryFromFirestore(language: String): List<QuizHistory> {
        ...
        val userId = auth.currentUser?.uid ?: return emptyList() // Dapatkan user ID dari FirebaseAuth

        val quizHistoryCollection = firestore.collection("users")
            .document(userId) // Gunakan userId dari auth
            .collection("quiz_history")

        val snapshot = quizHistoryCollection
            .whereEqualTo("language", language)
            .whereGreaterThanOrEqualTo("score", 10) // Misalnya, nilai lulus adalah skor >= 10
            .get()
            .await()
